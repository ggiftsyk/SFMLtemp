...
///////////////////////////////////////
bool solid[80][40];//tile block /size wall
std::vector<std::pair<int, int>> path; //for random path monster
...
{
	tinyxml2::XMLDocument levelFile;//загружаем файл в TiXmlDocument
	bool status = levelFile.LoadFile(filename.c_str());
	// загружаем XML-карту
	if (status)//если не удалось загрузить карту
	{
		...
		while (tileElement)
		{
			if (tileElement->Attribute("gid")) {
				...
				if (subRectToUse >= 0)
				{
					...
					if (layers.size() == 5 or layers.size() == 6) {
						solid[x][y] = true;
					}
			...
			{
				if (layers.size() == 4 or layers.size() == 5) {
					//path.push_back({ x,y });
				}
			}*/
...
void Level::Draw(sf::RenderWindow& window)
{
	...
	for (int layer = 0; layer < layers.size(); layer++)
	{
		...
		{
			//std::cout << layers[layer].tiles[tile];
